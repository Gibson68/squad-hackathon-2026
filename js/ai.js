...
const CAT_RULES = [
  { match: /customer payment/i, category: 'Sales',     color: '#27AE60' },
  { match: /stock|wholesale|inventory/i, category: 'Inventory', color: '#E89B2A' },
  { match: /rent/i, category: 'Rent',     color: '#6C5CE7' },
...
export async function chatRespond(message, _history = []) {
  ...
  if (/revenue|sales|income/.test(m)) {
    ...
    return `Your average monthly revenue is ₦${TRADER.monthlyRevenue.toLocaleString()}, growing ${TRADER.growth}% MoM. My forecast for next month is ~₦${f.toLocaleString()} — a healthy continuation of the upward trend you've held since December.`;
  }
